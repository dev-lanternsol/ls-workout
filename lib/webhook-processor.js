...
import { analyzeWorkoutWithCurl, analyzeWorkoutTextWithCurl } from '@/lib/gemini';
...
export async function processClickUpRaw(raw, opts = {}) {
  ...
  // Check if this is a workout reprocessing request
  if (typeof raw === 'object' && raw.reprocess && raw.workout) {
    return await reprocessWorkout(raw.workout, raw.original_workout_id);
  }
  ...
  for (const imageUrl of images) {
    ...
    let workoutData;
    try {
      workoutData = await analyzeWorkoutWithCurl(imageBase64, message);
    } catch (e) {
  ...
  if (hasText) {
    try {
      const textOnly = await analyzeWorkoutTextWithCurl(message);
...
// Function to reprocess an existing workout
async function reprocessWorkout(workout, originalWorkoutId) {
  const supa = getSupabaseAdmin();

  console.log(`Reprocessing workout ID ${originalWorkoutId}...`);
  console.log('Workout data:', workout);
  
  try {
    // Check if we have image_url or raw_message to reprocess
    const hasImage = workout.image_url && isValidHttpUrl(workout.image_url);
    const hasText = workout.raw_message && workout.raw_message.trim().length > 0;
    
    if (!hasImage && !hasText) {
      return {
        success: false,
        info: { reason: 'No image or text content available for reprocessing' }
      };
    }
    
    let workoutData = null;
    
    // Try to reprocess with image first if available
    if (hasImage) {
      try {
        const imageBase64 = await downloadImage(workout.image_url);
        if (imageBase64) {
          workoutData = await analyzeWorkoutWithCurl(imageBase64, workout.raw_message || '');
        }
      } catch (error) {
        console.log('Image reprocessing failed, trying text-only:', error.message);
      }
    }
    
    // If image processing failed or no image, try text-only
    if (hasText) {
      try {
        workoutData = await analyzeWorkoutTextWithCurl(workout.raw_message);
      } catch (error) {
        console.log('Text reprocessing failed:', error.message);
      }
    }
    ...
    if (!workoutData) {
      return {
        success: false,
        info: { reason: 'Failed to reprocess workout with both image and text analysis' }
      };
    }
    
    // Normalize and update the existing workout
    const normalized = normalizeWorkout(workoutData);
    ...
    if (updateError) {
      return {
        success: false,
        info: { reason: 'Failed to update workout', error: updateError.message }
      };
    }
    ...
    return {
      success: true,
      info: { 
        reason: 'Workout reprocessed successfully',
        workout_id: originalWorkoutId,
    ...
    return {
      success: false,
      info: { reason: 'Reprocessing error', error: error.message }
    };
  }
}
